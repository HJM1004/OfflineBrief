...
## 🗂 ライフログ(予定・ToDo・メモ管理アシスタント)

ニュースブリーフとは別に、`/life/` で予定・ToDo・メモを管理するPWAが使えます(2026年7月〜)。

- **予定**: ClaudeがGoogleカレンダー本体を直接読み書き。PWAには直近14日の予定を表示
- **ToDo・メモ**: Driveの `LifeLog/state.json` が正本。スマホのPWAから直接追加・完了・整理でき、
  Claudeとの会話(「あれやらなきゃ」「メモを整理して」「今日のまとめ」)でも操作できます
- **オフライン対応**: 閲覧はキャッシュ、オフライン中の追加・変更は再接続時に自動送信

セットアップは `scripts/gas-life/DEPLOY.md`(専用GAS Web Appのデプロイ、5分程度)。
Claude側の運用プロトコルは `CLAUDE.md` の「ライフログ」セクションに定義されています。
